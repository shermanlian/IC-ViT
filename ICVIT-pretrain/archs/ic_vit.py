...
class ChPatchEmbed(nn.Module):
    ...
    def forward(self, x, extra_tokens={}):
        cls_mask = extra_tokens['n_channels']
        B, C, H, W = x.shape
        ...
        x[batch_idx, cls_mask] += self.channel_cls_token.unsqueeze(0)
...
class SharedPatchEmbed(nn.Module):
    ...
    def forward(self, x, extra_tokens={}):
        B, C, H, W = x.shape
        x = x.view(B*C, 1, H, W)
        x = self.proj(x).flatten(2).transpose(1, 2).reshape(B, -1, self.embed_dim)
        return x


class IsolatedChannelViT(nn.Module):
    """ Efficient Channel Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.patch_embed = SharedPatchEmbed(
    ...
    def interpolate_pos_encoding(self, x, w, h):
        ...
        patch_pos_embed = patch_pos_embed.expand(1, self.in_chans, -1, dim).reshape(1, -1, dim)
    ...
    #def shrink_patches(self, x):
    #    cls_tokens = x[:, :1]
    #    x_reduced = x[:, 1:].view(
    #        x.shape[0], self.in_chans, -1, self.embed_dim
    #    ).mean(dim=1)
    #    x = torch.cat((cls_tokens, x_reduced), dim=1)
    #    return x
