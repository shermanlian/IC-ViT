...
import icvit.data as data
from icvit.meta_arch import Supervised
from wandb.sdk.wandb_settings import Settings
from pytorch_lightning.loggers import WandbLogger
from pytorch_lightning.utilities.rank_zero import rank_zero_only


# Ensure Wandb is only initialized in rank 0
@rank_zero_only
def setup_wandb_logger(cfg):
    # wandb.login()  # Ensure login (optional)
    return WandbLogger(
       # finish_previous=True,
        reinit=True,
        project="clean-code",
        name=f"{cfg.meta_arch.backbone.name}_full_run1",#[backbone/model name] - [patitial/full] - [run no].
        group=f"icvit",
        job_type=f"test",
        settings=Settings(init_timeout=480))

def wrap_collate_fn(original_collate_fn):
    def new_collate_fn(batch):
        inputs, labels = original_collate_fn(batch)
        inputs = inputs[:, :8, ...]
...
def get_train_loader(cfg: DictConfig):
    ...
    if len(cfg.train_data) == 1:
        ...
        #    train_data, **train_data_cfg.loader,collate_fn=wrap_collate_fn(train_data.collate_fn)
        )
...
def train(cfg: DictConfig) -> None:
    ...
    trainer = pl.Trainer(
        ...
        num_sanity_val_steps=0, callbacks=callbacks, **cfg.trainer
...
def predict(cfg: DictConfig) -> None:
    ...
    model = Supervised.load_from_checkpoint(cfg.checkpoint, weights_only=False)
    #model = Supervised.load_from_checkpoint(
    #cfg.checkpoint,
    #weights_only=False 
#)
    #model = torch.load(cfg.checkpoint, weights_only=False)
    ...
    val_loader = DataLoader(
        #val_data, **val_data_cfg.loader, collate_fn=custom_collate_fn    
