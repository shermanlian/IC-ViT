...
from icvit.backbone.vit import Block
from icvit.utils import trunc_normal_
...
class SharedPatchEmbed(nn.Module):
    ...
    def forward(self, x, extra_tokens={}):
        ...
        x = x.view(B*Cin, 1, H, W)
        x = self.proj(x)
        _, Cout, h, w = x.shape
        x = x.view(B, Cin, Cout, h, w).transpose(1, 2)
...
def icvit_tiny(patch_size=16, **kwargs):
...
def icvit_small(patch_size=16, **kwargs):
    model = IsolatedChannelViT(
...
def icvit_base(patch_size=16, **kwargs):
    model = IsolatedChannelViT(
