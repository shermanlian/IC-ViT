...
import icvit.backbone as backbone
import icvit.data as data
import icvit.utils as utils
...
class Supervised(pl.LightningModule):
    def __init__(self, main_supervised_cfg: DictConfig) -> None:
        ...
        if self.cfg.backbone.checkpoint is not None:
            print('use pretrained checkpoint: ', self.cfg.backbone.checkpoint)
            #checkpoint = torch.load(self.cfg.backbone.checkpoint, weights_only=True)
            checkpoint = torch.load(self.cfg.backbone.checkpoint, weights_only=False)
            ##########################记得加回来#################
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(f"train/loss", loss, on_step=True, prog_bar=True, rank_zero_only=True)
    ...
    def on_validation_epoch_end(self):
        ...
        for loader_idx, outputs_dict in val_outputs_per_loader.items():
            ...
            name = 'val' if loader_idx == 0 else 'test'
