...
class S3SharedClient:
    ...
    def get_image(
        ...
        path = file_path.replace('s3://insitro-research-2023-context-vit/jumpcp', '~/datasets/wsi/jumpcp')
        ...
        for _ in range(stop_max_attempt_number):
            try:
                s3_obj = self.s3_client.get_object(Bucket=bucket, Key=key, ChecksumMode='')
